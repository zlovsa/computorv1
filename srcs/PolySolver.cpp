...
void collect_polynom(const exprnode *root, std::map<double, Expression::VALUE_TYPE> &polynom, int sign = 1)
{
	...
	if (c == '+' || c == '-')
	{
		...
		polynom[0] += sign * root->value;
	else if (c == '^')
		polynom[root->right->value] += sign;
	else if (c == '*')
	{
		...
		double power = 1;
		if (pwr->opcode != 'v')
			power = pwr->right->value;
		polynom[power] += sign * coef->value;
	}
	else if (c == 'v')
		polynom[1] += sign;
}
...
bool poly_zero(std::map<double, Expression::VALUE_TYPE> polynom)
{
	for (auto i : polynom)
		if (i.second != 0)
			return false;
...
Expression::VALUE_TYPE sqrt(Expression::VALUE_TYPE d)
{
	if (d < 0)
		return -1;
	else if (d == 0)
		return 0;
	Expression::VALUE_TYPE r0, r1;
	r1 = 1;
	do
	{
		r0 = r1;
...
Expression::VALUE_TYPE abs(Expression::VALUE_TYPE x)
{
	if (x < 0)
		return (-x);
	return x;
...
std::string PolySolver::printPolynom(std::map<double, Expression::VALUE_TYPE> polynom, std::string varname) const
{
	...
	for (auto i : polynom)
	{
		if (i.second == 0 && !poly_zero(polynom))
			continue;
		Expression::VALUE_TYPE k = abs(i.second);
		if (!printplus)
		{
		...
		if (i.first == 0 || k != 1)
			ss << Expression::fixedout(k, _digits);
		if (i.first != 0)
		{
			if (k != 1)
				ss << " * ";
...
void PolySolver::solve()
{
	...
	else if (deg == 2)
	{
		...
		{
			std::stringstream ss;
			ss << "Discriminant is strictly negative, there is no real solutions." << std::endl;
			...
			{
				ss << "In case you may need complex ones:";
				addroot(re, -sqrt_d);
				addroot(re, sqrt_d);
			}
			else
